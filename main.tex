...
\begin{frame}{}
    ...
    \[
    k_{eff} = k - \frac{\partial F_{mag}}{\partial x}\bigg|_{x=x_{eq}}
    \]
...
\begin{frame}{Analyse des solutions}
    ...
    \[
    r = -\frac{\omega_0}{2Q} = -\frac{\alpha}{2m} \qquad \Omega = \omega_0 \sqrt{\frac{1}{4Q^2} - 1} = \sqrt{\frac{\alpha^2}{4m^2} - \frac{k_{eff}}{m}}
    \]
...
\begin{frame}{Oscillation forcée}
    ...
    \[
    F(t) = F_0 \cos(\omega t + \varphi_0)
    \]

    Equation différentielle associée:
    \[
    mx''(t) + \frac{m\omega_0}{Q} x'(t) + k x(t) = F_0 \cos(\omega t + \varphi_0)
    \]

    Solution particulière:
    \[
    \underline x_p(t) = X_0 e^{i(\omega t + \varphi_p)}
    \]
    % \[
    % \implies \underline x_p(t) = \frac{F_0\cos(\omega t + \varphi_0)}{(k-\omega^2 m) + i\alpha\omega}
    % \]
    % \[
    % \tan(\varphi_p) = \frac{\alpha\omega}{k-\omega^2 m}
    % \]

    \[
    \tan (\varphi_p) = - \frac{m\omega_0\omega}{Q(k - m\omega^2)}
    \]

    Avec aimant:
    \[
    \tan(\varphi_p) = \frac{\alpha\omega}{k - \partial -\omega^2 m}
    \]
\end{frame}
