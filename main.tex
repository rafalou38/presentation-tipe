...
\begin{frame}{Analyse des solutions}
    Solution homogène en amplitude:
    % \[
    % \underline x(t) = \underline X_h \exp{\left(\left[-\frac{\omega_0}{2Q} + i\omega_0 \sqrt{\frac{1}{4Q^2} - 1}\right]t\right)}
    % \]
    \[
    \underline x(t) = \underline X_h e^{(r+i\Omega)t} \qquad \text{avec } r = -\frac{\omega_0}{2Q} \text{ et } \Omega = \omega_0 \sqrt{\frac{1}{4Q^2} - 1}
    \]
...
\begin{frame}{Oscillation forcée}
    ...
    \[
    m x''(t) + \alpha x'(t) + k x(t) = F_0 \cos(\omega t + \varphi_0)
    \]
    ...
    \]
    \[
    \implies \tan(\varphi_p) = \frac{\alpha\omega}{k-\omega^2 m}
    \]
\end{frame}
